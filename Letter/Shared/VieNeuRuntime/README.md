...
a preset voice identifier and a per-request cancellation token, so chapter
changes can stop obsolete autoregressive inference without destroying the
loaded model. Exposing another bundled preset does not require changing the C++
inference implementation.

The vendored runtime was reduced to the ONNX preset-voice path and adapted for
the model's dynamic local-layer count, speaker embedding projection, and the
official MOSS frame-level streaming decoder state. Letter
...
failure recovery. The runtime still excludes reference-audio encoding and voice
cloning.
