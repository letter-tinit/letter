...
public enum BudgetMethod: String, CaseIterable, Hashable, Codable {
    ...
    public func generateBucketByIncome(_ income: Decimal) -> [BudgetBucket] {
        switch self {
        case .fiftyThirtyTwenty:
            return [
...
public struct BudgetBucket: Hashable {
    public let kind: BudgetBucketKind
    public let ratio: Decimal
    public let amount: Decimal
    public init(kind: BudgetBucketKind, ratio: Decimal, amount: Decimal) { self.kind = kind; self.ratio = ratio; self.amount = amount }
}

nonisolated
public enum BudgetBucketKind: Hashable, Codable {
    case needs
    case wants
    case savings
    case necessities
    ...
    public var supportsFixedExpensePlan: Bool {
        switch self {
