...
@MainActor
final class JSONPlaybackCheckpointRepository: PlaybackCheckpointRepository {
    ...
    private static func load(from url: URL?) -> [UUID: BookPlaybackCheckpoint] {
        ...
        return Dictionary(records.map { ($0.bookID, $0.checkpoint) }, uniquingKeysWith: { _, latest in latest })
    }
