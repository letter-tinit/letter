...
final class HabitStore {
    ...
    private let backupService = HabitBackupService()
    private let statisticsCalculator = HabitStatisticsCalculator()
    private let notificationScheduler = HabitNotificationScheduler()
...
extension HabitStore {
    ...
    func safetyBackups() -> [URL] {
        do {
            return try backupService.safetyBackups()
        } catch {
    ...
    func createPreImportBackup() throws -> URL {
        ...
        return try backupService.writePreImportBackup(data: data)
    }

    func importBackupData(_ data: Data) throws {
        _ = try createPreImportBackup()
        let backup = try backupService.decodeAndValidate(from: data)
