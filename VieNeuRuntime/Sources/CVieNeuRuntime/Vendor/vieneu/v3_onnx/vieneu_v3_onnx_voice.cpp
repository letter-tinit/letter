    ...
    try {
        ...
        for (auto it = presets.begin(); it != presets.end(); ++it) {
            ...
            if (item.contains("speaker_emb") && item.at("speaker_emb").is_array()) {
                for (const auto& value : item.at("speaker_emb")) {
                    preset.speaker_embedding.push_back(value.get<float>());
                }
            }
            if (config_.use_speaker_embedding &&
                preset.speaker_embedding.size() !=
                    static_cast<size_t>(config_.speaker_embedding_dim)) {
                error = "VieNeu v3 preset voice is missing a valid speaker_emb: " + id;
                return false;
            }
